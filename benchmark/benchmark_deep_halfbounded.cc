...
int main(int argc, char **argv)
{
    ...
    for (unsigned index_k : index_k_list)
    {
        for (unsigned ef_construction : ef_construction_list)
        {
            ...
            i_params.K = index_k;
            {
                ...
                BaseIndex::SearchInfo search_info(&data_wrapper, &i_params, "SeRF",
                                                  "benchmark");
    ...
    // USE 5 points in the benchmark: 0.1% 0.5% 1% 5% 10% 50% 100%
    // Not storing the meta results, just output the statics
