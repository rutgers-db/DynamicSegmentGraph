...
public:
    ...
    mutable size_t candidate_from_out_bound;
    mutable size_t candidate_from_in_bound;
    mutable size_t num_search_comparison;

    mutable double decompress_neighbor_time;
    ...
    // We need to change into a map not an unordered_map because we need lower_bound
    std::map<labeltype, tableint> label_lookup_;
...
//                    if (candidate_id == 0) continue;
    ...
    void AccumulateTime(timeval &t1, timeval &t2, double &val_time) const {
    ...
    std::priority_queue<std::pair<dist_t, tableint>,
                        std::vector<std::pair<dist_t, tableint>>,
                        CompareByFirst>
    searchBaseLayerST(tableint ep_id, const void *data_point, size_t ef, const bool fixed_ef, const int lbound, const int rbound, const int K_query) const {
        candidate_from_in_bound = 0;
        ...
        // if (!has_deletions || !isMarkedDeleted(ep_id)) {
        //     dist_t dist = fstdistfunc_(data_point, getDataByInternalId(ep_id),
        //     dist_func_param_); lowerBound = dist;
        //     // top_candidates.emplace(dist, ep_id);
        //     candidate_set.emplace(-dist, ep_id);
        // } else {
        //     lowerBound = std::numeric_limits<dist_t>::max();
        ...
        // original: set the ep from above layer
        // candidate_set.emplace(-lowerBound, ep_id);
        // visited_array[ep_id] = visited_array_tag;

        // set enter points as range index
        {
            int lb = lbound;
            int interval = (rbound - lb) / 10;
            for (size_t i = 0; i < 10; i++) {
                int point = lb + interval * i;
                auto it = label_lookup_.lower_bound(point);
                if (it != label_lookup_.end() && visited_array[it->second] != visited_array_tag) {
                    const auto& internal_id = it->second; // Key found, access the value
                    char *currObj1 = (getDataByInternalId(internal_id));
                    dist_t dist = fstdistfunc_(data_point, currObj1, dist_func_param_);
        ...
        size_t num_dist_comparison = 0;

        while (!candidate_set.empty()) {
            std::pair<dist_t, tableint> current_node_pair = candidate_set.top();
            if ((-current_node_pair.first) > lowerBound && top_candidates.size() == search_ef) {
                break;
            }

#ifdef LOG_DEBUG_MODE
            std::cout << "current node: "
                      << getExternalLabel(current_node_pair.second) << "  -- "
                      << -current_node_pair.first << std::endl;
#endif
...
//                    if (candidate_id == 0) continue;
                ...
                if (!(visited_array[candidate_id] == visited_array_tag)) {
                    ...
                    // AccumulateTime(tt1, tt2, decompress_neighbor_time);
#ifdef LOG_DEBUG_MODE
                    // std::cout << "candidate: " << getExternalLabel(candidate_id)
                    //           << "  dist: " << dist << std::endl;
#endif
                    num_search_comparison++;
                    if (top_candidates.size() < search_ef || lowerBound > dist) {
                        candidate_set.emplace(-dist, candidate_id);
#ifdef USE_SSE
                        _mm_prefetch(
                            data_level0_memory_ + candidate_set.top().second * size_data_per_element_ + offsetLevel0_, ///////////
                        ...
                        if (acutal_id <= rbound && acutal_id >= lbound) {
                            ...
                            if (getExternalLabel(current_node_id) <= rbound) {
                                candidate_from_in_bound++;
                            } else {
    ...
    std::priority_queue<std::pair<dist_t, tableint>> searchKnnInternal(
        ...
        if (has_deletions_) {
            std::priority_queue<std::pair<dist_t, tableint>> top_candidates1 =
                searchBaseLayerST<true>(currObj, query_data, ef_, fixed_ef, lbound,
                                        rbound, K_query);
            top_candidates.swap(top_candidates1);
        } else {
            std::priority_queue<std::pair<dist_t, tableint>> top_candidates1 =
                searchBaseLayerST<false>(currObj, query_data, ef_, fixed_ef, lbound,
                                         rbound, K_query);
            top_candidates.swap(top_candidates1);
        }
     ...
     * whereas maxM0_ has to be limited to the lower 24 bits, however, still large
        ...
        for (int layer = 0; layer <= elemLevel; layer++) {
            ...
            for (auto &&neigh : sNeigh) {
                ...
                std::priority_queue<std::pair<dist_t, tableint>,
                    ...
                    candidates;
                size_t size =
                    sCand.find(neigh) == sCand.end() ? sCand.size() : sCand.size() - 1; // sCand guaranteed to have size >= 1
    ...
    std::priority_queue<std::pair<dist_t, labeltype>> searchKnnEF(
        ...
        const int rbound,
        const int K_query,
        const bool fixed_ef = false) const {
