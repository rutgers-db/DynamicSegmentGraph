...
for i in 1; do
    ...
    LOG_PATH="${root_path}log/Serf_Stream_${dataset}_${INDEX_SIZE}_${index_k}_${ef_max}_${ef_construction}.log"

    # Iterate over methods and run the benchmark

    echo "Running benchmark for dataset: $dataset"
    echo "./benchmark/scalibility -N $N -k $index_k -ef_construction $ef_construction -ef_max $ef_max -dataset $dataset -dataset_path $dataset_path -query_path $query_path"

    ./benchmark/scalibility -N $N -k $index_k -ef_construction $ef_construction -ef_max $ef_max \
