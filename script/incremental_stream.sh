...
for i in $(seq 1 $((${#DATASETS[@]} - 1))); do
    ...
    LOG_PATH="${root_path}log/incremental_stream/${dataset}/OuterLog_${INDEX_SIZE}_${index_k}_${ef_max}_${ef_construction}.log"

    # Iterate over methods and run the benchmark

    echo "Running incre_stream for dataset: $dataset"
    echo "./benchmark/incre_stream -N $N -k $index_k -ef_construction $ef_construction -ef_max $ef_max -dataset $dataset -dataset_path $dataset_path -query_path $query_path"
