...
//                    if (candidate_id == 0) continue;
  ...
  searchBaseLayerST(tableint ep_id, const void *data_point, size_t ef,
                    const bool fixed_ef, const int lbound, const int rbound,
                    const int K_query) const {
    ...
    // set enter points as range index
    {
      ...
      for (size_t i = 0; i < 10; i++) {
        ...
        auto internal_id = label_lookup_.at(point);
...
//                    if (candidate_id == 0) continue;
  ...
  std::priority_queue<std::pair<dist_t, tableint>> searchKnnInternal(
      void *query_data, int k, const int lbound, const int rbound,
      const int K_query, const bool fixed_ef) {
  ...
  std::priority_queue<std::pair<dist_t, labeltype>> searchKnnEF(
      const void *query_data, size_t k, const int lbound, const int rbound,
      const int K_query, const bool fixed_ef = false) const {
