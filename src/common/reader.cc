...
/// @param N Reading top N vectors
/// @param num_dimensions dimension of dataset
void ReadIvecsTopN(const std::string &file_path, std::vector<int> &keys, const uint32_t N, const int num_dimensions, const int position) {
    ...
    int counter = 0;
    while ((counter < N) && ifs.read((char *)buff.data(), num_dimensions * sizeof(uint64_t))) {
...
void ReadDataWrapper(vector<vector<float>> &raw_data, vector<int> &search_keys, const string &dataset, string &dataset_path, const int item_num) {
    raw_data.clear();
    if (dataset == "glove") {
...
void ReadDataWrapper(const string &dataset, string &dataset_path, vector<vector<float>> &raw_data, const int data_size, string &query_path, vector<vector<float>> &querys, const int query_size, vector<int> &search_keys) {
    raw_data.clear();
    if (dataset == "glove" || dataset == "glove25" || dataset == "glove50" || dataset == "glove100" || dataset == "glove200") {
...
void ReadMatFromTxt(const string &path, vector<vector<float>> &data, const int length_limit = -1) {
    ifstream infile;
    string bline;
    string delim = " ";
    int numCols = 0;
...
void ReadMatFromTxtTwitter(const string &path, vector<vector<float>> &data, const int length_limit = -1) {
    ifstream infile;
    string bline;
    string delim = " ";
    int numCols = 0;
...
void ReadMatFromTsvYT8M(const string &path, vector<vector<float>> &data, vector<int> &search_keys, const int length_limit) {
    ifstream infile;
    string bline;
    string delim = ",";
    int numCols = 0;
...
void ReadMatFromTsvYT8M(const string &path, vector<vector<float>> &data, const int length_limit) {
    ifstream infile;
    string bline;
    string delim = ",";
    int numCols = 0;
...
// load data and querys
void ReadDataWrapper(const string &dataset, string &dataset_path, vector<vector<float>> &raw_data, const int data_size) {
    raw_data.clear();
    if (dataset == "glove" || dataset == "glove25" || dataset == "glove50" || dataset == "glove100" || dataset == "glove200") {
        ReadMatFromTxtTwitter(dataset_path, raw_data, data_size);
    } else if (dataset == "ml25m") {
