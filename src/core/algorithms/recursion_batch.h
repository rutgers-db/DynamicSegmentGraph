 ...
 * @brief 动态段图(DSG)递归批处理算法实现
 * 这个文件实现了基于HNSW的范围过滤搜索算法，支持任意范围查询
 * 主要功能：
 * 1. 扩展HNSW算法，支持递归添加邻居节点
 * 2. 实现范围过滤搜索，只在指定范围内计算距离
 * 3. 使用批处理方式管理前向和反向邻居关系
 * @date 2023-06-19
 *
 ...
 */
 ...
 namespace rangeindex
 {
    ...
    /**
     * @brief 支持范围过滤的HNSW算法类
     * 
     * 这个类扩展了标准的HNSW算法，增加了以下功能：
     * 1. 递归添加邻居节点，提高图的连通性
     * 2. 支持有向邻居关系管理（前向和反向）
     * 3. 批处理方式管理邻居节点，提高查询效率
     * 
     * @tparam dist_t 距离类型（通常为float）
     */
    ...
    public:
        ...
        vector<DirectedNeighbors> *directed_nns;           // 有向邻居数组指针
        vector<vector<pair<int, float>>> reverse_nns_vecs; // 反向邻居向量，存储(节点ID, 距离)对
 
        /**
         * @brief 启发式剪枝算法，限制邻居数量
         * 
         * 重写了HNSW的启发式剪枝方法，支持ef_for_pruning参数
         * 该方法从候选节点中选择最优的M个邻居，使用贪心策略：
         * 1. 优先选择距离查询点较近的节点
         * 2. 避免选择彼此过于接近的节点（多样性）
         * 
         * @param top_candidates 候选节点优先队列（距离，节点ID）
         * @param M 最大邻居数量
         */
        ...
        /**
         * @brief 递归连接新元素到图中（核心算法）
         * 
         * 这是DSG算法的核心方法，实现了递归批处理的邻居连接策略：
         * 1. 首先连接剪枝后的最优邻居
         * 2. 递归地添加剩余候选节点，形成多个批次
         * 3. 每个批次记录起始和结束位置，用于范围查询优化
         * 4. 同时维护前向和反向邻居关系
         * 
         * @param data_point 新节点的数据指针
         * @param cur_c 当前节点的内部ID
         * @param top_candidates 候选邻居优先队列
         * @param level 层级（0表示底层）
        ...
        virtual tableint mutuallyConnectNewElementLevel0(
            ...
            int level, bool isUpdate)
         {
            ...
            std::unordered_set<tableint> selectedNeighbors_set; // 已选择的邻居集合，避免重复
            selectedNeighbors_set.reserve(M_);

            size_t Mcurmax = level ? maxM_ : maxM0_; // 当前层的最大邻居数

            // 第一步：启发式剪枝，选择最优的M_个邻居
            ...
            // 记录最左边的位置，用于递归算法的位置选择策略
            int external_left_most = -1;
            vector<int> addedNeighborPositions; // 记录已添加邻居的位置
            ...
            while (((recursion_counter++ < 100) && (!addedNeighborPositions.empty())))
             {
                ...
                case BaseIndex::IndexParams::Recursion_Type_t::MIN_POS:
                    external_left_most = addedNeighborPositions.front(); // 选择最小位置作为分割点
                    break;
                case BaseIndex::IndexParams::Recursion_Type_t::MID_POS:
                    external_left_most =
                        addedNeighborPositions[addedNeighborPositions.size() / 2]; // 选择中间位置
                    break;
                case BaseIndex::IndexParams::Recursion_Type_t::MAX_POS:
                    external_left_most = addedNeighborPositions.back(); // 选择最大位置
                    break;
                case BaseIndex::IndexParams::Recursion_Type_t::SMALL_LEFT_POS:
                    external_left_most = before_prune_cancidates.top().second; // 选择距离最远的节点位置
                }
                ...
                while (!before_prune_cancidates.empty())
                {
                    ...
                    if (external_nn > external_left_most) // 只保留位置大于分割点的节点
                    {
    ...
    /**
     * @brief 递归批处理索引类 - DSG算法的主要实现
     * 
     * 这个类实现了完整的动态段图(DSG)算法，包括：
     * 1. 索引构建：使用RangeFilteringHNSW构建图结构
     * 2. 范围查询：支持任意范围内的近似最近邻搜索
     * 3. 批处理优化：通过批次管理提高查询效率
     * 4. 递归策略：支持多种递归添加邻居的策略
     */
    class RecursionIndex : public BaseIndex
    {
    public:
        vector<DirectedNeighbors> directed_indexed_arr; // 有向邻居数组，每个节点对应一个DirectedNeighbors
         ...
         * @param data 数据包装器
         */
        RecursionIndex(base_hnsw::SpaceInterface<float> *s, const DataWrapper *data)
            : BaseIndex(data)
        {
        ...
        base_hnsw::DISTFUNC<float> fstdistfunc_; // 距离计算函数指针
        void *dist_func_param_;                  // 距离函数参数

        VisitedListPool *visited_list_pool_; // 访问列表池，用于搜索过程中标记已访问节点
        IndexInfo *index_info;               // 索引信息统计
