 ...
 * @param real_search_key_range 真实搜索键范围
 * @param K_neighbor K邻居数
 * @param search_time 搜索时间
 * @param gt 地面实况
 * @param pts 点数据
 */
void SaveToCSVRow(const string &path, const int idx, const int l_bound, 
                  const int r_bound, const int pos_range, const int real_search_key_range, 
                  const int K_neighbor, const double &search_time, 
                  const vector<int> &gt, const vector<float> &pts) {
    std::ofstream file;
...
public:
    ...
    void generateRangeFilteringQueriesAndGroundtruth(bool is_save = false, const string path = "") {
        ...
        for (auto range : query_range_list) {
            ...
            for (int i = 0; i < this->querys.size(); i++) {
                ...
                if (is_save) {
                    SaveToCSVRow(path, i, l_bound, r_bound, range, search_key_range,
        ...
        if (is_save) {
            cout << "Save GroundTruth to path: " << path << endl;
        }
    }
    ...
    void generateHalfBoundedQueriesAndGroundtruth(bool is_save = false, const string path = "") {
        ...
        if (this->data_size == 100000) {
            // 小范围查询配置，用于精细性能测试
            query_range_list = {10, 20, 40, 80, 160, 320, 640};
    ...
    void generateRangeFilteringQueriesAndGroundtruthBenchmark(bool is_save_to_file, const string save_path = "") {
        ...
        // 使用小范围配置，与DynamicSegmentGraph原始项目保持一致
        // query_range_list.emplace_back(10);
        // query_range_list.emplace_back(20);
        // query_range_list.emplace_back(40);
        // query_range_list.emplace_back(80);
        // query_range_list.emplace_back(160);
        // query_range_list.emplace_back(320);
        // query_range_list.emplace_back(640);
